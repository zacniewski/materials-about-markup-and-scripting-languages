...
1. Aktywuj je i zainstaluj bibliotekę `requests` (`pip install requests`).
1. Stwórz plik `hello.py`, który wypisze w konsoli "Witaj w Pythonie".
1. Uruchom skrypt komendą `python hello.py`.
1. Wygeneruj plik `requirements.txt`.
1. Sprawdź wersję zainstalowanego Pythona i biblioteki `requests` za pomocą odpowiednich poleceń w terminalu.
1. Zdezaktywuj środowisko wirtualne i spróbuj uruchomić skrypt `hello.py` ponownie, obserwując czy coś się zmieniło.
1. Usuń folder `venv` i spróbuj go ponownie utworzyć z inną nazwą, np. `.my_env`.
1. Zainstaluj bibliotekę `pandas` w nowym środowisku i sprawdź czas instalacji.
1. Stwórz prosty skrypt, który pobierze dane od użytkownika (imię) i wyświetli powitanie "Cześć [imię], gratulacje z okazji konfiguracji środowiska!".
