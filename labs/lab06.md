...
1. Rozbuduj klasę `Student` o listę ocen. Dodaj metody `dodaj_ocene(ocena)` oraz `oblicz_srednia()`.
1. Stwórz klasę `KontoBankowe` z atrybutem `saldo`. Dodaj metody `wplata(kwota)` i `wyplata(kwota)`. Zadbaj o to, by saldo nie było ujemne.
1. Stwórz klasę bazową `Zwierze` z metodą `daj_glos()`. Stwórz klasy `Pies` i `Kot`, które dziedziczą po `Zwierze` i nadpisują metodę `daj_glos()`.
1. Stwórz prosty system zarządzania biblioteką: klasy `Ksiazka` i `Biblioteka`. `Biblioteka` powinna zawierać listę obiektów `Ksiazka`.
1. Dodaj do klasy `Samochod` atrybuty `marka`, `model` oraz `rok_produkcji`. Stwórz metodę, która oblicza wiek samochodu na podstawie aktualnego roku.
1. Zaimplementuj klasę `Prostokat` z atrybutami `dlugosc` i `szerokosc`. Dodaj metody obliczające pole oraz obwód prostokąta.
1. Stwórz klasę `Pracownik` z atrybutem `wynagrodzenie`. Stwórz klasę `Menadzer`, która dziedziczy po `Pracownik` i dodaje atrybut `premia`. Dodaj metodę obliczającą całkowite zarobki menadżera.
1. Zaimplementuj metodę specjalną `__str__` dla klasy `Ksiazka`, aby wypisywała informacje o książce w formacie: "Tytuł: [tytuł], Autor: [autor]".
1. Stwórz klasę `Punkt2D` z atrybutami `x` i `y`. Napisz metodę obliczającą odległość między dwoma punktami.
1. Stwórz klasę `Kwadrat`, która dziedziczy po klasie `Prostokat`. Nadpisz konstruktor tak, aby przyjmował tylko jeden parametr `bok`.
1. Dodaj do klasy `Pracownik` atrybut prywatny `__pesel`. Stwórz właściwość (`@property`), która pozwoli na odczyt peselu, ale nie na jego zmianę.
1. Stwórz listę obiektów różnych klas (np. `Kolo`, `Prostokat`, `Kwadrat`), gdzie każda ma metodę `pole()`. W pętli wypisz pola wszystkich tych figur (demonstracja polimorfizmu).
1. Stwórz klasę `Produkt` (nazwa, cena) oraz `Koszyk`. `Koszyk` powinien umożliwiać dodawanie produktów i obliczać całkowitą wartość zamówienia.
1. Zaimplementuj klasę `Samochod` z atrybutami prywatnymi `__marka` i `__predkosc`. Dodaj metody `przyspiesz()` i `zwolnij()`, które modyfikują prędkość, ale nie pozwalają jej spaść poniżej 0. Wykorzystaj `@property` do odczytu prędkości.
