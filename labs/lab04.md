...
1. **`sorted(iterable)`**: Tworzy **nową, posortowaną listę** z dowolnego obiektu iterowalnego (lista, krotka, słownik). Oryginał pozostaje bez zmian.
...
1. Napisz skrypt, który pobiera od użytkownika 5 liczb, zapisuje je w liście, a następnie wypisuje średnią arytmetyczną tych liczb.
1. Mając słownik: `ceny = {"chleb": 4.50, "mleko": 3.20, "jajka": 10.00}`, oblicz sumę zakupów na podstawie listy produktów podanych przez użytkownika.
1. Napisz program, który usunie duplikaty z listy: `[1, 2, 2, 3, 4, 4, 5, 1]` przy użyciu zbioru (`set`).
1. Napisz program zliczający wystąpienia każdego słowa w podanym zdaniu.
1. Stwórz listę krotek, gdzie każda krotka zawiera imię i wiek osoby. Napisz program, który posortuje tę listę według wieku.
1. Napisz program, który łączy dwa słowniki w jeden, przy czym w przypadku powtarzających się kluczy, wartości powinny zostać zsumowane.
1. Napisz program, który znajduje największy i najmniejszy element w liście liczb bez użycia funkcji `max()` i `min()`.
1. Stwórz słownik, w którym kluczami są numery indeksów studentów, a wartościami ich oceny. Napisz program, który wyliczy średnią ocen wszystkich studentów.
1. Napisz program, który pobiera od użytkownika dwa zbiory liczb i wypisuje ich część wspólną oraz różnicę symetryczną.
1. Stwórz listę słowników reprezentujących pracowników (imię, nazwisko, pensja). Napisz program, który przy użyciu funkcji `sorted()` i `lambda` posortuje tę listę według pensji malejąco.
1. **Analiza tekstu (Indeksy słów):** Napisz program, który na podstawie tekstu wprowadzonego przez użytkownika (np. `"kot pije mleko i kot pije wodę"`) stworzy słownik, gdzie kluczem będzie słowo, a wartością lista pozycji (indeksów), na których to słowo wystąpiło w tekście.
