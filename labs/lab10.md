...
1. Stwórz folder `templates` i plik `index.html`. Użyj funkcji `render_template`, aby wyświetlić ten plik we Flasku.
...
1. Dodaj drugi widok pod adresem `/kontakt`, który wyświetli prosty formularz HTML.
1. Dodaj do aplikacji obsługę metody `POST` dla widoku `/kontakt`. Po przesłaniu formularza, aplikacja powinna wyświetlić komunikat: "Dziękujemy za kontakt, [imię]!".
...
1. Dodaj obsługę błędów (np. błąd 404), wyświetlając własną stronę błędu.
