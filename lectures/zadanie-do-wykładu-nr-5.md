...
2. Podając imię i wiek.
3. Podając imię, wiek i miasto.
...
2. Użyj funkcji `map()` i wyrażenia `lambda`, aby stworzyć nową listę, w której każda liczba z oryginalnej listy zostanie pomnożona przez 2.
...
2. Zaimportuje moduł `random` (używając aliasu `rd`) i wylosuje liczbę całkowitą z zakresu od 1 do 100.
3. Wyświetli oba wyniki.
